# restatum
> Enjoy managing your shared/global state with restatum
...
### Reducer sample
Above, we uses `React.useState` approach. This is the simple form of manaing your state with restatum. 
...
function Todos() {
    ...
    return (
        <ul>{todos.map(todo => <li key={todo}>{todo}</todo>)}</ul>
    )
}
```
...
stores near to the Components who consume them. As much as possible, provide a scope to your shared state
...
you could make a best choice what behaviuor you want to manage your state. Slicing your state could leverage
this feature.
...
Every key to the configuration are equivalent to store.
...
`StoresProvider` holds the stores. Only the Components which are included to the tree can access any stores. 
`StoresProvider` accepts an optional `initialStoresState`. If the prop is given, then the value passed will 
override the `initialState` from the `configuration` object. It accepts the same type in `initialState` or 
...
to the state chagne inside the Component, via passing this object as an argument to the thooks.
...
bound to the state. Means, the Component will rerender  whenever there is stata change.
